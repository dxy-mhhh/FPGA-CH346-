module rd_rst(
    ...
    output          wire_data_oe        ,
    output          wire_slv_state_ready,
    output          wire_slv_rdrst_retry,
...
//--data check
reg reg_slv_state_data;
reg reg_rst_retry;
always @(posedge wire_clk or negedge wire_rstn) begin
    if(~wire_rstn) begin
        reg_slv_state_data <= 0;
    end
    // else if(~wire_cs_n) begin
	else if(wire_rd_pos) begin
		if(reg_data_cnt == 1)begin
			reg_slv_state_data <= (wire_data_tmp == 0);
        end
	end
    // end
    else begin
...
always @(posedge wire_clk or negedge wire_rstn) begin
    if(~wire_rstn) begin
        ...
        // reg_rst_retry <= ~wire_cs_n & wire_rd_pos;
        reg_rst_retry <= wire_rd_pos;
	end
end
...
reg reg_slv_state_ready;
reg reg_slv_rdrst_retry;
// always @(posedge wire_clk or negedge wire_rstn) begin
    // if(~wire_rstn) begin
        // reg_slv_state_ready <= 0;
    // end
    // else if(reg_slv_state_data)
        // reg_slv_state_ready <= 1;
// end
always @(posedge wire_clk or negedge wire_rstn) begin
    if(~wire_rstn) reg_slv_state_ready <= 0;
    else reg_slv_state_ready <= reg_slv_state_data;
end


always @(posedge wire_clk or negedge wire_rstn) begin
    if(~wire_rstn) reg_slv_rdrst_retry <= 0;
    else reg_slv_rdrst_retry <= reg_rst_retry;
end

assign  wire_slv_state_ready = reg_slv_state_ready;
assign  wire_slv_rdrst_retry = reg_slv_rdrst_retry;
