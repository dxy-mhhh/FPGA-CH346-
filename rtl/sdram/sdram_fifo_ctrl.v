...
module sdram_fifo_ctrl(
    ...
    //Bank状态输出
    output            bank_wr_ready,     //Bank可写状态信号，1表示有Bank可写
    output [9:0]      rdf_rdusedw        //读端口FIFO中的数据量
    );
...
reg        bank0_read_done;              //Bank0读完成标志
reg        bank1_read_done;              //Bank1读完成标志
reg        current_read_bank;            //当前读Bank标志
reg        write_waiting;                //写操作等待状态
...
//Bank可写状态判断
//下一个要写入的Bank是~rw_bank_flag
//如果该Bank已经读完（bank_read_done为1），或者两个Bank都未写入过（初始状态），则可写
assign bank_wr_ready = (~rw_bank_flag == 1'b0) ? bank0_read_done : bank1_read_done;
assign rdf_rdusedw = rdf_use;
...
always @(posedge clk_ref or negedge rst_n) begin
    if (!rst_n) begin
        bank0_read_done <= 1'b0;
        bank1_read_done <= 1'b0;
    end
    else if(wr_load_flag) begin              //写端口复位时清除标志
        bank0_read_done <= 1'b0;
        bank1_read_done <= 1'b0;
    end
    else if(sw_bank_en) begin                //Bank切换完成时，清除对应Bank的读完成标志
        //rw_bank_flag现在已经更新到新的Bank了
        if(rw_bank_flag == 1'b0)
            bank0_read_done <= 1'b0;
        else
            bank1_read_done <= 1'b0;
    end
    else if(read_done_flag && sdram_pingpang_en && 
            sdram_rd_addr[22:0] >= rd_max_addr - rd_length) begin
        //读操作完成一个Bank时，设置对应Bank的读完成标志
        if(sdram_rd_addr[23] == 1'b0)
            bank0_read_done <= 1'b1;
        else
            bank1_read_done <= 1'b1;
    end
end
...
always @(posedge clk_ref or negedge rst_n) begin
    if (!rst_n) begin
        ...
        if(sdram_pingpang_en) begin          //SDRAM 读写乒乓使能
            if(sdram_wr_addr[22:0] < wr_max_addr - wr_length) begin
                sdram_wr_addr <= sdram_wr_addr + wr_length;
            end
            else begin                       //准备切换BANK
                // 检查下一个Bank是否已经读完
                if((~rw_bank_flag == 1'b0 && bank0_read_done) || 
                   (~rw_bank_flag == 1'b1 && bank1_read_done)) begin
                    // 对应Bank已读完，可以安全切换
    ...
    else if(write_waiting) begin             // 写操作正在等待读完成
        // 检查对应Bank是否已读完
        if((~rw_bank_flag == 1'b0 && bank0_read_done) || 
           (~rw_bank_flag == 1'b1 && bank1_read_done)) begin
            // 现在可以切换了
...
always @(posedge clk_ref or negedge rst_n) begin
    if(!rst_n) begin
        ...
        if(sdram_pingpang_en) begin          //SDRAM 读写乒乓使能  
            if(sdram_rd_addr[22:0] < rd_max_addr - rd_length) begin
...
always@(posedge clk_ref or negedge rst_n) begin
    if(!rst_n) begin
        ...
        if(write_waiting) begin          //写操作正在等待读完成
                                         //优先执行读操作，让读尽快完成
            if((rdf_use < rd_length)     //若读端口FIFO中的数据量小于读突发长度，
